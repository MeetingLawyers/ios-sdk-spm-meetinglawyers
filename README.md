...
Add one library per Xcode target:

- `MeetingLawyers` — main SDK, for the app target.
- `MeetingLawyersNSE` — Notification Service Extension, for the extension target.
- `MeetingLawyersCore` — shared core, from 2.2.0 on. The two libraries above already link it; add it
  only to link it on its own. It is not re-exported, so `import MeetingLawyersCore` is required to
  use its APIs.
